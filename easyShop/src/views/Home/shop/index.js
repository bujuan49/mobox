import React from 'react';
import style from "./index.module.scss"
import {observer,inject} from "mobx-react"
@inject("shop")
@observer
class shop extends React.Component {
    constructor(props) {
        super(props);
        this.state = {};
    }
    componentDidMount(){
        this.props.shop.getallShop()
    }
    render() {
        const {allShop} =this.props.shop;
        console.log(allShop)
        return (
            <React.Fragment>
                <div className={style.shopwrap}>
                    <ul className={style.serviceList}>
                        <li>
                            <span role="img">★</span>
                            30天无忧退货
                        </li>
                        <li>
                            <span role="img">★</span>
                            48小时快速退款
                        </li>
                        <li>
                            <span role="img">★</span>
                            满88元免邮费
                        </li>
                    </ul>
                    <div className={style.shop_content}>
                        {
                            allShop.cartList&&allShop.cartList.map(item=>
                                <div className={style.shop_box} key={item.goods_id}>
                                    <div><img src={ 
                                        item.checked==1 ?
                                    "data:image/png;base64," 
                                    :"data:image/png;base64,"
                                    } alt=""/></div>
                                    <div><img src={item.list_pic_url} alt=""/></div>
                                    <div>
                                        <div>
                                            <h4>{item.goods_name}</h4>
                                            <p>{item.retail_price}</p>
                                        </div>
                                    </div>
                                    <div>{item.number}X</div>
                                </div>
                            )
                        }
                    </div>
                    <div className={style.shop_price}>
                        <div className={style.allCheckout}><img src={
                            allShop.cartTotal&&allShop.cartTotal.checkedGoodsCount==1?
                            "data:image/png;base64,"
                            :"data:image/png;base64,"
                        } alt=""/></div>
                        <div className={style.cartCount}>
                           <p>已选(<b>{allShop.cartTotal&&allShop.cartTotal.goodsCount}</b>)</p><span>￥{allShop.cartTotal&&allShop.cartTotal.checkedGoodsAmount}</span>
                        </div>
                        <div>编辑</div>
                        <div className={style.cartAllBtn}>下单</div>
                    </div>
                </div>
            </React.Fragment>
        );
    }
}
export default shop;
//data:image/png;base64,